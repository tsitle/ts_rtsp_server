...
final class ReadEsRawFileMetaOLDDD {

	private ReadEsRawFileMetaOLDDD() { }

	static void readMeta(
				@NonNull String extEsId,
				@NonNull RtspConfigElementaryStreamSourceOLDDD ioEsSource
			) throws ConfigInvalidException {
		if (! ioEsSource.getEnabled()) {
			return;
		}
		switch (ioEsSource.getCodec()) {
			case RtpPacketType.A_AAC -> ReadEsRawFileMetaOLDDD.readMeta_aac(extEsId, ioEsSource);
			case RtpPacketType.A_AC3 -> ReadEsRawFileMetaOLDDD.readMeta_ac3(extEsId, ioEsSource);
			case RtpPacketType.V_H264 -> ReadEsRawFileMetaOLDDD.readMeta_h264Header(extEsId, ioEsSource);
			case RtpPacketType.V_H265 -> ReadEsRawFileMetaOLDDD.readMeta_h265Header(extEsId, ioEsSource);
		}
	}
