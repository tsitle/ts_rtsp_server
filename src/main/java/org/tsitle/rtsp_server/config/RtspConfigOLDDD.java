 ...
 */
public final class RtspConfigOLDDD {
	...
	@Expose
	private @Nullable Map<@NonNull String, @NonNull RtspConfigElementaryStreamSourceOLDDD> elementaryStreamSources;
	/** Map of Muxed-Stream Sources (the map keys are unique Muxed-Stream Source identifiers) */
	@Expose
	private @Nullable Map<@NonNull String, @NonNull RtspConfigMuxedStreamSourceOLDDD> muxedStreamSources;
	/** Map of Input Sources (the map keys are unique Input Source identifiers) */
	@Expose
	private @NonNull Map<@NonNull String, @NonNull RtspConfigInputSourceOLDDD> inputSources;
	...
	@GsonAnnoExclude
	private @NonNull Map<@NonNull Integer, @NonNull RtspConfigElementaryStreamSourceOLDDD> internalEsSources;
	...
	@GsonAnnoExclude
	private @NonNull Map<@NonNull Integer, @NonNull RtspConfigMuxedStreamSourceOLDDD> internalMsSources;
	...
	public @NonNull Set<String> getUsersAllowedToAccessInputSource(@NonNull RtspProtoIdInputSource idInputSource) {
		checkPostProcessed();
		//
		Optional<RtspConfigInputSourceOLDDD> tmpOptIsObj = getInputSourceObj(idInputSource);
	...
	/**
	 * Get a list of all Elementary-Stream Source IDs.
	 * @return Elementary-Stream Source IDs
	 */
	public @NonNull List<@NonNull Integer> getElementaryStreamSourceIds() {
		checkPostProcessed();
		return List.copyOf(
				internalEsSources.keySet().stream().sorted().toList()
			);
	}
	 ...
	 */
	public Optional<RtspConfigElementaryStreamSourceOLDDD> getElementaryStreamSourceObj(int esSourceIdAsInt) {
		checkPostProcessed();
	 ...
	 * @return Elementary-Stream Source object
	 */
	public Optional<RtspConfigElementaryStreamSourceOLDDD> getElementaryStreamSourceObj(@NonNull RtspProtoIdEsSource idEsSource) {
		checkPostProcessed();
		//
		for (RtspConfigElementaryStreamSourceOLDDD tmpSs : internalEsSources.values()) {
	 ...
	 */
	public Optional<RtspConfigMuxedStreamSourceOLDDD> getMuxedStreamSourceObj(int msSourceIdAsInt) {
		checkPostProcessed();
		//noinspection OptionalOfNullableMisuse,DataFlowIssue
	...
	public Optional<RtspConfigMuxedStreamSourceOLDDD> getMuxedStreamSourceObj(@NonNull RtspProtoIdMsSource idMsSource) {
		checkPostProcessed();
		//
		for (RtspConfigMuxedStreamSourceOLDDD tmpMs : internalMsSources.values()) {
	 ...
	 * @return Input Source
	 */
	public Optional<RtspConfigInputSourceOLDDD> getInputSourceObj(@NonNull String inputSourceId) {
		checkPostProcessed();
		//noinspection OptionalOfNullableMisuse,DataFlowIssue
	 ...
	 * @return Input Source
	 */
	public Optional<RtspConfigInputSourceOLDDD> getInputSourceObj(@NonNull RtspProtoIdInputSource idInputSource) {
		checkPostProcessed();
		//
		for (RtspConfigInputSourceOLDDD tmpIs : inputSources.values()) {
	...
	public void postProcess() throws ConfigInvalidException {
		...
		for (int tmpEsId : tmpEsIdList) {
			RtspConfigElementaryStreamSourceOLDDD tmpEsObj = getElementaryStreamSourceObj(tmpEsId).orElseThrow();
		...
		for (int tmpMsId : tmpMsIdList) {
			RtspConfigMuxedStreamSourceOLDDD tmpMsObj = getMuxedStreamSourceObj(tmpMsId).orElseThrow();
		...
		for (String tmpIsId : tmpIsIdList) {
			RtspConfigInputSourceOLDDD tmpIsObj = getInputSourceObj(tmpIsId).orElseThrow();
	...
	private Optional<String> getAbsoluteFilePath(@NonNull String errMsg, @Nullable String filename) throws ConfigInvalidException {
		...
		String resStr = RtspConfigElementaryStreamSourceOLDDD.dataFilenameToAbsolutePath(getDataDirAsPath(), filename);
	...
	private void createInternalEsSourcesMap() {
		...
		int idCounter = 10;
		for (Map.Entry<String, RtspConfigElementaryStreamSourceOLDDD> entry : elementaryStreamSources.entrySet()) {
			if (entry.getKey() == null || entry.getValue() == null) {
				continue;
			}
	...
	private void createInternalMsSourcesMap() {
		...
		for (Map.Entry<String, RtspConfigMuxedStreamSourceOLDDD> entry : muxedStreamSources.entrySet()) {
			if (entry.getKey() == null || entry.getValue() == null) {
				continue;
			}
	...
	private void validateSectionEsSources() throws ConfigInvalidException {
		final String FNC_NAME = getClass().getSimpleName() + ".validateSectionEsSources()";
		...
		for (int tmpSsId : tmpSsIdList) {
			RtspConfigElementaryStreamSourceOLDDD tmpSsObj = getElementaryStreamSourceObj(tmpSsId).orElseThrow();
	...
	private void createVirtualEsForMsSources() throws ConfigInvalidException {
		...
		for (String tmpIsId : getInputSourceIds()) {
			...
			RtspConfigMuxedStreamSourceOLDDD tmpMsSrcObj = getMuxedStreamSourceObj(tmpMsIdAsInt).orElseThrow();

			FfmpegDmxSubStreamInfoVideo tmpFfSsInfoVid = tmpMsSrcObj.getFfSubStreamInfoVideo();
			if (tmpFfSsInfoVid.ffmpegCodec.isVideo()) {
				RtspConfigElementaryStreamSourceOLDDD tmpEsSrcObj = RtspConfigElementaryStreamSourceOLDDD.createFromDemuxedSubStreamVideo(
						idCounter,
						tmpMsIdAsInt,
			...
			if (tmpFfSsInfoAud.ffmpegCodec.isAudio()) {
				RtspConfigElementaryStreamSourceOLDDD tmpEsSrcObj = RtspConfigElementaryStreamSourceOLDDD.createFromDemuxedSubStreamAudio(
						idCounter,
						tmpMsIdAsInt,
						tmpMsSrcObj.getInputUri(),
						tmpFfSsInfoAud
	...
	private void addVirtualEsToIsSources(
				@NonNull RtspConfigInputSourceOLDDD isSrcObj,
				int msSrcIdAsInt,
				@NonNull RtspConfigElementaryStreamSourceOLDDD esSrcObj
			) {
