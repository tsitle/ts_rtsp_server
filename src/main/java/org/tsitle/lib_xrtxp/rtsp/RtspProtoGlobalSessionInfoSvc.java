...
/**
 * Service for global RTSP Session Information.
 */
public final class RtspProtoGlobalSessionInfoSvc implements RtspProtoGlobalSessionInfoInterface {
	...
	public @NonNull RtspProtoIdSubStream createSubStreamId(
		...
		try {
			...
			if (tmpTries == MAX_TRIES) {
				throw new IllegalStateException("Failed to generate unique Sub-Stream ID");
			}
	...
	private @NonNull SubStreamResolve getResolveBySubStreamId(
		...
		final String ipStr = clientIpAddr.getIpAddrStr().orElseThrow(() ->
				new IllegalArgumentException("Client IP address must be set")
			);
