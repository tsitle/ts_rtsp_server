...
public final class FfmpegDemuxer implements AutoCloseable {
	...
	private static @NonNull String sanitizeTagString(@NonNull String tagValue) {
		return tagValue
				.replace("ö", "oe").replace("ø", "oe").replace("œ", "oe")
				.replace("Ö", "Oe").replace("Ø", "Oe").replace("Œ", "Oe")
				.replace("ä", "ae").replace("å", "ae").replace("æ", "ae")
				.replace("Ä", "Ae").replace("Å", "Ae").replace("Æ", "Ae")
				.replace("ü", "ue").replace("Ü", "Ue")
				.replace("ß", "ss")
				.replace("ñ", "nj").replace("Ñ", "Nj")
				.replace("`", "'").replace("´", "'")
				.replace("“", "\"").replace("”", "\"")  // left/right double quotation mark
				.replace("„", "\"")  // lower double quotation mark
				...
				.replace("ỳ", "y").replace("Ỳ", "Y")
				.replaceAll("\\p{Cc}", "");  // remove control chars
	}
