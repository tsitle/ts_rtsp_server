...
public final class ParamsThreadDpPcm implements Cloneable {
	...
	@Override
	public @NonNull ParamsThreadDpPcm clone() {
		try {
			return (ParamsThreadDpPcm)super.clone();
	...
	private void validateParamValues() {
		...
		if (audioChannelCount < 1 || audioChannelCount > DpConstants.DP_PCM_AUDIO_CHANNELS_MAX) {
			throw new IllegalArgumentException(errPrefix + "audioChannelCount must be >= 1 and <= " +
					DpConstants.DP_PCM_AUDIO_CHANNELS_MAX);
		}
	}

	private static void requireIsSet(boolean v, String name) {
		final String errPrefix = ParamsThreadDpPcm.class.getSimpleName() + ": ";
